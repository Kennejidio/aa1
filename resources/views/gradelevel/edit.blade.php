...
@section('content')
    ...
    <section class="d-flex justify-content-center my-5">
        ...
        <!-- ! FORM -->
        <form action="{{ route('gradelevel.update', $gradelevel->id) }}" method="POST"
            class="bg-white rounded-1 shadow p-3
        mb-5">
            @csrf
            @method('PUT')
            <div class="text-center">
                <h2 class="text-darkpink">EDIT GRADE LEVEL</h2>
            </div>
            @if ($errors->any())
            ...
            <div class="row mt-4">
                <div class="col-md input-group mt-1">
                    ...
                    <input class="form-control" type="text" name="gradelevel" value="{{ $gradelevel->code }}">
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md input-group mt-1">
                    <textarea class="form-control" cols="30" rows="2" name="description">{{ $gradelevel->description }}</textarea>
                </div>
            </div>
